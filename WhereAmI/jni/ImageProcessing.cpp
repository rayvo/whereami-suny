...
extern "C" jboolean Java_com_suny_ocr_ImageProcessingActivity_extractFrags(
	...
	for (i = 0; i < len; i++) {
		...
		Mat tmp_gr = Mat::zeros(max_x - min_x + 1, max_y - min_y + 1, CV_8UC1);
		Mat tmp_bn = Mat::zeros(max_x - min_x + 1, max_y - min_y + 1, CV_8UC1);
		int width = temp.cols;
		...
		/*IplImage* img = cvCreateImage(temp.size(), IPL_DEPTH_8U, 3);    //rgb
				*img = temp;
		...
		cvMerge(b, g, r, NULL, img);  //merge all separate channels together to output image rgb */

		/*cvtColor( temp, tmp_gr, CV_RGB2GRAY );

		threshold(tmp_gr, tmp_bn,  122 , 255 , THRESH_BINARY);

		cvtColor( tmp_bn, temp, CV_GRAY2RGB );*/
		cv::Mat greyMat;
		cv::cvtColor(temp, greyMat, CV_BGR2GRAY);
		...
		minMaxLoc( greyMat, &minVal, &maxVal, &minLoc, &maxLoc );

		add(greyMat, -1*minVal, greyMat);

		greyMat = (255/(maxVal-minVal))*greyMat;

		cv::cvtColor(greyMat, temp, CV_GRAY2BGR);
